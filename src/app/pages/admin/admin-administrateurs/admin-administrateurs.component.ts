...
export class AdminAdministrateursComponent implements OnInit {
  ...
  loadAdmins() {
    this.http.get<any[]>('http://192.168.57.230:8080/utilisateurs')
      .subscribe({
  ...
  ajouterAdmin() {
    ...
    this.http.post('http://192.168.57.230:8080/utilisateurs', formData)
      .subscribe({
        next: () => {
          this.form.reset();
          this.loadAdmins();
  ...
  supprimer(id: number) {
    if (confirm("Voulez-vous vraiment supprimer cet admin ?")) {
      this.http.delete(`http://192.168.57.230:8080/utilisateurs/${id}`)
        .subscribe({
