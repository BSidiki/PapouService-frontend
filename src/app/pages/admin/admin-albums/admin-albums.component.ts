...
export class AdminAlbumsComponent implements OnInit {
  ...
  loadAlbums() {
    this.http.get<any[]>('http://192.168.244.230:8080/albums').subscribe(data => {
  ...
  onSubmit() {
    ...
    const request = this.isEditing
      ? this.http.put(`http://192.168.244.230:8080/albums/${this.editId}`, formData)
      : this.http.post('http://192.168.244.230:8080/albums', formData);
  ...
  deleteAlbum(id: number) {
    if (confirm('Supprimer cet album ?')) {
      this.http.delete(`http://192.168.244.230:8080/albums/${id}`).subscribe(() => this.loadAlbums());
    }
  }
}
