...
export class AdminAlbumsComponent implements OnInit {
  ...
  onSubmit() {
    ...
    const request = this.isEditing
      ? this.http.put(`http://192.168.11.100:8080/albums/${this.editId}`, formData)
      : this.http.post('http://192.168.11.100:8080/albums', formData);
  ...
  deleteAlbum(id: number) {
    if (confirm('Supprimer cet album ?')) {
      this.http.delete(`http://192.168.11.100:8080/albums/${id}`).subscribe(() => this.loadAlbums());
    }
  }
}
